...
- **小脳の垂れ下がりは 2026-09-08 に取り直した。2026-08-28 の数値は撤回する。** 旧セッションは K を
  ...
  **保持デューティの計器は小脳オフの垂れ下がりであって、ff の読み値ではない** — ff は
  `--cerebellum-cf-deadband` の設定しだいで経路依存になる。そして **2 関節が `--cerebellum-ff-max`
  に達した件は、水増しではなかった**: 健全なレールでも `elbow_flex` は 315 を要求し、300 のクランプに
  当たる。**正常動作で当たるクランプは、正常と異常を区別できない。** 上限は置き直しが要る（未着手 —
  315 は 1 姿勢の値で、他の姿勢での正当な最大値は測っていない）。
