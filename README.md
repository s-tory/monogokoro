...
- **フィードフォワードは収束せず減衰した。修正済み。ただし修正値は未実測。** どちらの学習ランでも、
  ...
  **摩擦帯は電源が落ちたことの副産物ではない** ── 2026-09-09 に切り分けた: レールが健全
  （平均 4.44-4.51 V、最小 4.40 V）でも帯は出る。そして帯の幅を測った ── 同じ目標に上から/下から近づけると、
  `shoulder_lift` は 18.0 カウント（デューティ 120 と 480）、`elbow_flex` は 12.9 カウント
  （デューティ 300 と 106）離れた場所で静止する。**帯は保持デューティ本体と同じオーダーある。**
  同一設定の 4 ラン（すべて標準偏差 0.00）でも保持デューティは 100/180/200/255 とばらついた。
  **`err = pwm/K` が成立することも標準偏差が 0 であることも、釣り合いの証拠にはならない** ──
  前者は経路に PD 則しか無ければ恒等式で、後者は静止しか言わず、静止は釣り合いと固着を区別しない。
