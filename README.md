...
- **ポリシーの推論と小脳が同じ iGPU を共有した状態は測っていない。** iGPU のコンピュートキューは
  1 本きりでグラフィックスと共用なので、小脳のサブミットは他の仕事の後ろに並ぶ。アイドル時に平均
  307 µs だった 1 ステップは、デスクトップが動いているだけで最大 2969 µs まで振れる（[実測値であって仮定ではない](#実測値であって仮定ではない)）。
  ACT を約 30 Hz で回しながらの値は無い。反射のほうは seqlock と `--cerebellum-staleness-ms`（既定
  200 ms、200 Hz なら 40 周期ぶん）に守られて止まらないが、ゲートが破棄するまでのあいだ古い予測が
  出続ける帯で何が起きるかは分かっていない。測る相手は推論であって学習ではない — アームを動かしながら
  学習する運転状態がそもそも無いからである。
