...
今は上から**天然ゴムの指サック**（滑り止めのドット付き）を被せてある —— 理由と付け方は
[指先のシリコンは 2 週間で破れた](README_DETAILS.md#指先のシリコンは-2-週間で破れた)。
...
| 関節ごとの K              | **3/8/3/1.3/1/1**                               | 7.4 V・小脳オンで、値を伏せて手で押してもらい決めた（盲検）。以前の「K=1 で保持させた PWM ＝重力+摩擦のデューティ」から逆算する方法は捨てた — 止まっている関節は、釣り合いと静摩擦の引っかかりを区別できない |
...
- [指先のシリコンは 2 週間で破れた](README_DETAILS.md#指先のシリコンは-2-週間で破れた) — ゴムが保つかは未測定。
...
- [単発の bit0 は 5 V では残り、7.4 V で消えた](README_DETAILS.md#単発の-bit0-は-5-v-では残り74-v-で消えた) — 脱力時、603 秒で 0 本。
- [7.4 V の電源を作った](README_DETAILS.md#74-v-の電源を作った) — 余裕をさらに取るなら。はんだ付けが要る。
