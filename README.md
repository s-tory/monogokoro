...
- [付属の電源では既定設定でレールが落ちる](README_DETAILS.md#付属の電源では既定設定でレールが落ちる) — 329 秒に 1 回、833 ms。**既製品の 5 V 6 A に差し替えれば既定条件では消える**（プラグを挿すだけ、測定済み）。さらに余裕が要るなら [7.4 V の電源を作った](README_DETAILS.md#74-v-の電源を作った)（はんだ付けが要る）。
