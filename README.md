...
- **駆動中にバスの通信エラーが出る。同一設定のランで 0〜43%。原因は未特定。**
  2026-09-09 に `fault_flags` をログに出して初めて見えた。エラーは **約 800 ms の固定長**
  （20 ms サンプルで 39-41 個）のバーストで来て、その 75% は **PWM 飽和の 60 ms 以内**に始まる。
  バーストが `--max-blind-ticks` を超えるとデューティはゼロに落ち、腕が下がる ── これは
  デューティの列だけ見ると「K が柔らかすぎる」と区別がつかず、実際そう誤読していた。
  **腕を脱力させると同じ機体で 0.00%** なので、引き金は駆動であってバス自体ではない。
  `--current-read-divisor 4` は tick を 16% 短くしてもエラー率を変えないので混雑ではない。
  `--serial-timeout-ms 2` は**悪化させる**（2 ms は届くはずの返信を切り、遅れた返信が次の問いの
  答えとして読まれる）。800 ms は `Protection_Time` / `Over_Current_Protection_Time`（全モータ 200）
  が 4 ms 単位なら一致する ── **未検証**だが、当たりならサーボ自身の過負荷保護が PWM モードで
  発火していることになる。**この制約の上で測った数値は、すべてこの影響を受けている。**
