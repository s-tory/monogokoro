...
- **小脑单步的耗时，在与推理交错的情况下没有测过。** 策略这一侧测了（见[实测值而非假设](#实测值而非假设)）：
  ACT 单次前向是 30 ms，而 `n_action_steps` 默认是 100，所以 30 Hz 的机器人每 3.3 秒才推理一次 ——
  占用率 0.9%，单次突入不到 `--cerebellum-staleness-ms`（默认 200 ms）的六分之一，就算排在它后面，
  也够不到陈旧预测仍被采用的那一段。**例外是 temporal ensembling，它会把 `n_action_steps` 强制为 1**：
  每个控制步都来一次前向，占用率逼近 90%。那种情况没有测。
