...
现在在外面再套一层**天然橡胶指套**（表面带防滑颗粒）来保护它 —— 原因和套法见
[指尖的硅胶两周就裂开了](README_DETAILS_CN.md#指尖的硅胶两周就裂开了)。
...
之后用 `--robot.type=so101_impedance_follower` 去做遥操作或录制即可。两者都会从机器人配置里自动填入
每个关节的 K/D。**这些默认值是在 7.4 V 电源上（舵机读数 7.0-7.1 V）通过盲测定下的。**
占空比是相对电源电压的比例，所以在 5 V 下同样的 K 输出的力只有大约 2/3，手感和保持都应该会变；
5 V 下还没有重新定过。
...
| 每关节的 K         | **3/8/3/1.3/1/1**                         | 在 7.4 V、小脑开启的条件下，不告诉数值、让手来推着判定（盲测）。以前"以 K=1 保持时上报的 PWM 就是重力+摩擦所需的占空比"的反推做法已放弃 —— 静止的关节分不清是平衡还是被静摩擦卡住 |
...
- [指尖的硅胶两周就裂开了](README_DETAILS_CN.md#指尖的硅胶两周就裂开了) —— 橡胶是否更耐用尚未测量。
