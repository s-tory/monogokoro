...
- **小脑与策略推理共用同一块集成 GPU 的情形，从未测过。** 计算队列只有一条，而且与图形共用，所以
  小脑的提交只能排在别的活儿后面。空闲时平均 307 us 的单步，仅仅因为桌面在动就会摆到最大 2969 us
  （见[实测值而非假设](#实测值而非假设)）；至于 ACT 以约 30 Hz 跑起来时是多少，没有数。反射两边都有
  保护 —— seqlock 加上 `--cerebellum-staleness-ms`（默认 200 ms，在 200 Hz 下相当于 40 个周期）——
  但在门限把它丢弃之前，陈旧的预测仍在被采用的那一段里会发生什么，尚不清楚。该测的是推理而不是训练：
  并不存在一边让手臂动、一边让训练占着 GPU 的运行状态。
