...
- **只有一个舵机拉起 bit0 的那一类，电源换了也没有消失。** 这一类每次只持续一个控制周期
  ...
  **「只有一个舵机拉起来就一定是过热或过载」这个说法已于 2026-09-14 撤回**。
  究竟是舵机看到了共享读数看不到的瞬时下陷，还是 bit0 根本不是电压（位与含义的对应关系来自二手资料，
