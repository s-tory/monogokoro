...
**代わりに言えるのは上限だけ。** 5 V 6.2 A のアダプタは**無負荷の端子で 5.18 V**、
サーボを繋いだアイドルで**サーボ側が 4.9 V**。条件が違うので引き算はできない ── 差の 0.28 V には
配線の降下とアダプタ自身の負荷ドループが混ざっている。だが**負荷が乗って端子電圧が上がることは
ないので、配線＋コネクタの降下は 0.28 V 以下**と言える（`端子(負荷時) − 4.9 ≤ 5.18 − 4.9`）。
