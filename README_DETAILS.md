...
は過負荷）** で、ケースは最高 33℃ にしか達していない。電圧ビット `0x01` は一度も立たなかった。
...
**2026-09-16、7.4 V では 78 分の稼働で単発がゼロ本だった**（デーモン 03:16:53-04:35:48 UTC、
`Present_Voltage` 70-71）。5 V 6 A での無負荷の頻度（0.165〜0.200 回/秒）が続いていれば、4735 秒で約 780〜950 本出ている計算になる。
**ただしこれは比較にならない** ── 上の 0.200 は**脱力・無負荷で条件を揃えて取った**数字で、
2026-09-16 の 78 分には駆動・保持・脱力が混ざっており、**その比率を記録していない。**
専用に設計した測定ではなく、別の作業のログから拾ったもの。
**傾向とは整合する**（レールが上がるほど減り、4.6→4.9 V で 0.688→0.200）。
**決着させるのは簡単で、まだやっていない**: 腕を脱力させたまま放置し、ケース温度を揃えて数える
── 上の 2 本と同じ手順を 7.4 V で 1 回。それが出るまで、この節の見出しは変えない。
