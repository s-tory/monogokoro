...
_測っていない。_ サーボを設計電圧で回す。STS3215 は **7.4 V** のサーボを標準構成が 5 V で
回しているもので、ベンダは **6〜7.4 V** と書き、定格トルクは 6 V と 7.4 V でのみ示されて
**5 V では一度も示されない**。範囲の下端で回すとトルクを失い、失ったトルクは電流で払う ──
レールを引き下げている当のものである。このアームの `Max_Voltage_Limit` は 80（8.0 V）なので
7.4 V は受け入れる範囲の内側で、トリップ点までの距離が 0.9 V から 3.4 V になる。
**律速もそれに連れて移るはず**: `Max_Temperature_Limit` は 70℃ で、5 V でも負荷時の肩は
すでに 35℃ に達している。**この段落はこのリグでの測定ではない** ── 7.4 V の電源はまだ一度も
動かしていない。データシートが指す方向であり、型番より持ち運びやすい（DC-DC モジュールは
入力を選ばないが、下のアダプタは 100 V 専用）。
