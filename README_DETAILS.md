...
**サーボ 6 個を繋ぐと、無負荷から約 0.3 V 下がる** ── 端子 5.18 V（テスター、2026-09-18）→
サーボ側 4.9 V（`Present_Voltage`、2026-09-14）。別の日・別の道具で、`Present_Voltage` は 0.1 V 刻み
なので「約」。**どこで落ちているか（配線か、アダプタの負荷ドループか）は未分離。**
7.4 V 側の 0.3〜0.4 V はレールも電流も違うので、これでは説明できない。
（以前ここにあった「5 V では降下がほぼ無い」は交換前のアダプタの値による誤り。2026-09-18 に撤回。）
