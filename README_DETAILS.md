...
### 2. リーダー側の力覚フィードバックは実装したが撤回した（2026-09-24）

`--leader-port` でリーダーのグリッパを触覚ディスプレイとして駆動する経路が一時存在した。撤回した理由は
2つ、どちらもこの個体で測ってはいない。**バス競合が未解決だった**——`--leader-port` は Python 側の
`so101_leader` teleoperator が既に開いている**同じシリアルポート**を、ロックも調停もなく別プロセスから
もう一度開いていた。デーモン冒頭のコメントは「SO101 の単一半二重バスを排他的に所有する」と謳っていたが、
それはフォロワ側にしか成り立っていなかった。**静止摩擦に埋もれると見込まれた**——STS3215 のギアボックス
自体の固着が、トリガに乗せられる力を上回ると予想された。ハードウェアについての仮定であって測定ではない。

再挑戦するなら、バス所有権を先に直すこと（ロックを入れるか、リーダーのグリッパもフォロワ同様
共有メモリ経由に載せ替える）——ポートを二重に開く前提から作り直す必要がある。
...
**むき出しのシリコン指サックは 2 週間ほどで裂けた**（2026-09-16 に発見）。裂けたのは物に当たる面で、
左が 2 箇所、右が大きく 1 箇所。

**引っ張って被せると徐々に裂ける。少し緩めに被せること。** 手で扱っての所見で、
締め具合を変えた A/B は取っていない。
...
今は上から**天然ゴムの指サック**（滑り止めのドット付き）を被せてある。ゴムのほうが保つかどうかは
まだ分からない — 交換が 2026-09-16 で、比べられるだけの時間が経っていない。

**材料寿命はここで測っていた軸ではなかった。** デッドバンドも剛性も接触の返り方も指先込みで測って
あるが、その指先が 2 週間で別物になる。定数に日付が要る理由がもう一つ増えた。
...
リーダーグリッパの力覚フィードバックは前半を直す候補の一つだったが撤回した
（→「リーダー側の力覚フィードバックは実装したが撤回した」の節）。デモンストレーション間の分散から
剛性を導く手は、まだ手つかずのまま残っている。
...
最低 4.50 V。**無負荷のアダプタ端子は 5.18 V**（テスター、2026-09-18 に実測。付属アダプタの同条件
4.97 V と並ぶ数字）。同じ pose・同じゲイン・400 Hz のエラーバイトで数えて、既定では
...
**サーボが読む 7.0〜7.1 V は、電源出力の 7.4 V より 0.3〜0.4 V 低い。** 5 V のときも同じ形で、
サーボ 6 個を繋ぐと無負荷から**約 0.3 V** 下がっていた ── 端子 5.18 V（テスター、2026-09-18）に対し
サーボ側 4.9 V（`Present_Voltage`、2026-09-14）。別の日・別の道具で、`Present_Voltage` は 0.1 V 刻み
なので「約」。

**どちらも、どこで落ちているかは分かっていない。** 配線とコネクタの降下と見ているが、アダプタ自身の
負荷ドループと分けるには、**サーボを繋いだまま電源出力端とサーボ側を同じ瞬間に読む**（テスター 1 回、
未実施）。
...
`$HF_LEROBOT_CALIBRATION/robots/`(既定 `~/.cache/huggingface/lerobot/calibration/robots/`)の下で
別のディレクトリに書く: 素のロボットは`so_follower/<id>.json`、こちらは`so101_impedance_follower/<id>.json`。

```bash
# 先にデーモンを止める。でないと2つのプロセスがシリアルポートを取り合う。
...
(`cp`を省いて`--robot.calibration_dir`に`so_follower`のディレクトリを直接指しても良い。)どちらもせずに
接続すると`NotImplementedError`が同じコマンドを出す——手書きではなく`self.calibration_dir`から生成して
いる(`so101_impedance_follower.py`の`calibrate()`参照)ので、この段落自体が今日のリネームで一度古くなった
ようなパスのズレは起きない。
