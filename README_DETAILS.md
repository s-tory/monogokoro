...
**ここには以前「5 V のときはアダプタ端 4.59 V に対しサーボが 4.6 V で降下がほぼ無かったので、
同じ配線なら説明がつかない」と書いてあった。2026-09-18 に撤回した。** 理由は三つ:

- **4.59 と 4.6 を同時に読んだ記録がどこにもない。** 4.59 はテスターでアダプタ端子、4.6 は
  アイドルの `Present_Voltage`。別々に測った数字を並べて「降下がほぼ無い」と読んでいた
- **4.59 は交換前の付属アダプタの値。** 5 V 電源は 2026-09-14 に 6.2 A のものへ替えてある
- **その新しい電源の値は、このページの上のほうに書いてある** ── アイドル 4.9 V。
  撤回した文は、同じ文書の中にある新しい数字を飛び越えて古いほうを取りに行っていた

**言えるのは合計だけ。サーボ 6 個を繋ぐと、無負荷から約 0.3 V 下がる** ──
5 V 6.2 A のアダプタは無負荷の端子で **5.18 V**、繋いだアイドルでサーボ側が **4.9 V**。
「約」なのは精度がそこまでだから: 5.18 はテスターで 2026-09-18、4.9 はサーボの `Present_Voltage` で
2026-09-14、**別の日・別の道具**で、しかも `Present_Voltage` は 0.1 V 刻み。

**その 0.3 V がどこで落ちているかは分かれていない。** 配線とコネクタの降下か、アダプタ自身の
負荷ドループか。負荷が乗って端子電圧が上がることはないので、**配線側はこれ以下**とだけ言える。

**これは 7.4 V 側の 0.3〜0.4 V を説明しない。** レールも電流も違うので直接は比べられない。
比べるには、**サーボを繋いだまま**アダプタ端子とサーボ側を同じ瞬間に読む必要がある（未実施、
テスター 1 回）。
