...
> Measured on an SO-101 with the 7.4 V servos, 2026-09-10 and 2026-09-14:
>
...
> - How much room there is above 4.0 V is a property of the supply, not of the arm. On the bundled
>   adapter the servos idled at **4.6 V**, so the margin was **0.6 V**, and a shoulder lifting
>   against gravity closed it: all six motors raised the voltage bit together for **833 ms** at a
>   time while a once-per-second supply reading still said 4.5 V. On a regulated 5 V 6.2 A brick
>   (2026-09-14, same arm and pose) they idle at **4.9 V**, hold **4.8 V** against gravity, and the
>   lowest seen while a hand pushed the arm around was **4.50 V**.
...
> this fork carry a `servo_error` column and a `FAULT_SERVO_ERROR` flag, and the daemon logs every
> change of the byte per motor at the loop rate. `0x01` on **every motor in the same tick** is the
> supply, because under-voltage is shared.
>
> `0x01` on **one motor alone** is a different thing, and what it is has not been established. It
> is not simply over-heat or over-load: on this arm it appears on the gripper servo about once
> every six seconds with the arm limp, no duty commanded, a 27 C case and 0.9 V of headroom —
...
>
> _Measured._ A bigger 5 V supply: **regulated 5 V, 6 A or more, 5.5×2.1 mm centre-positive** — the
...
> voltage does on the way there); note that a label will not tell you, so this comes off a datasheet
> or a product page. Swapping to one on 2026-09-14 took a 180 s gravity hold at the defaults from
> **one 833 ms collapse in 329 s to nothing at all over 5 ms in 233 s**. It is not a complete fix:
> at `--pwm-max 700` a **390 ms** collapse still happened, against 8 over 100 ms before. Each
> condition is a single run and the two adapters were not alternated, so treat the direction as
> established and the size as approximate.
>
...
> accept, and it moves the trip point from 0.9 V away to 3.4 V away. Expect the binding constraint
> to move with it: `Max_Temperature_Limit` is 70 C and the loaded shoulder already reached 35 C at
> 5 V. **None of that paragraph is a measurement on this rig** — no 7.4 V supply has been run here.
> It is the direction the datasheet points, and it travels better than a part number, because a
> DC-DC module takes any input while the brick below is 100 V only.
>
...
>
> The one measured here, for the record: **`LTE36ES-S1-301`** (Li Tone Electronics), 5 V 6.2 A,
> 31 W, sold by Akizuki as catalogue number 111105 for about ¥2,300. **Japan only, and not merely
> on availability — its input is 100 V.**
