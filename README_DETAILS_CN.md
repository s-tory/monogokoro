...
**它不再是负载的测量值，而变成「过渡过程冲到了哪里」的函数**。程序本身的默认值仍是 5。**systemd 单元从 2026-09-17 起用 8 启动** —— 推导和上面一样是
「1 个计数 × 关节刚度」，因为出厂增益里最大的是 lift 的 K=8。8 能不能消除振荡还没有测过。传 `0` 会回到旧行为。
...
**裸用的硅胶指套用了大约两周就裂开了**（2026-09-16 发现）。裂口在接触物体的那一面，左边两处，
右边一处比较大。

**绷得太紧套上去会慢慢裂开，套的时候留一点松。** 这是上手摆弄得到的观察，
没有做过松紧的 A/B。
...
现在在外面套了一层**天然橡胶指套**（表面带防滑颗粒）。橡胶是不是更耐用，**还不知道** —— 换上是
2026-09-16，还没有攒够可以比较的时间。

**材料寿命不是这里测量时考虑过的一个轴。** 死区、刚性、接触的回馈方式，都是带着这副指尖测出来的，
而这副指尖两周就变成了另一个东西。常数需要日期，又多了一个理由。
...
**`0x20`（按未经确认的位映射是过载）**，电压位 `0x01` 只在切断电源的那一刻出现过一次（6 个电机同时）。
...
7.4 V 上，余量是 3.0 V）；**空载时适配器端子 5.18 V**（万用表，2026-09-18 实测，与随机适配器同
条件下的 4.97 V 对应）；抵抗重力保持时 4.8 V；用手推动手臂时
...
### 单周期的 bit0 换 5 V 电源后还在，到 7.4 V 消失了

**只有一个舵机拉起 bit0 的那一类，换了 5 V 电源也没有消失。** 这一类每次只持续一个控制周期
...
**2026-09-17 在 7.4 V 下按同样的步骤做了一遍，一次都没有出现。** 只启动守护进程
（不接客户端，占空比指令为零），手臂收起并放松，**603 秒（约 24 万个 tick）**，
`Present_Voltage` 70-72，外壳温度 29-33℃。按 5 V 6 A 时每秒 0.165-0.200 次算，本该出现 100-120 次。
0 次意味着频率在 95% 置信度下低于每秒 0.005 次（3/603）—— **比 5 V 6 A 至少少 33 倍。**
外壳比 5 V 时最凉的那次（27℃）更热，而温度只会让频率变高，所以温度解释不了。
**这是手臂放松时的结果，驱动中的单周期事件没有测。**
（启动后马上有一个来自 `motor 5` 的损坏数据包，这是唯一一次通信错误。）
**还有一个条件没对齐：这次是非实时运行，而 5 V 的两次都是 `SCHED_FIFO`**（重新构建时 setcap 丢了）。
之前测过，有没有实时调度，单周期事件都是每秒 0.21 次，所以它不太可能解释这里的 0 次，但条件并没有对齐。

**2026-09-16 的「78 分钟一次都没有」把时间窗弄错了。** 04:29:02 电源被切断，此后总线上没有任何通信，
单周期事件根本不可能被看到。能观测的只有 03:16:53-04:29:02 的 **4329 秒**，
期望值约 710-870 次（这一节曾先后写成「约 800 次」和「4735 秒里 780-950 次」）。
其中驱动、保持和放松混在一起，比例也没有记录，所以它只能作为上面那次测量的旁证。
...
**这里原来写着「5 V 时适配器端 4.59 V、舵机读到 4.6 V，几乎没有压降，所以如果线路相同，
这个差值是解释不通的」。2026-09-18 撤回**，有三个原因:

- **没有任何记录表明 4.59 和 4.6 是同一时刻读的。** 4.59 是万用表量适配器端子，4.6 是空闲时的
  `Present_Voltage`。两次分开的测量被并排放在一起，读成了「几乎没有压降」
- **4.59 是换掉之前那个随机适配器的值。** 5 V 电源已在 2026-09-14 换成 6.2 A 的那只
- **换上之后的数值就写在本页上面**：空闲 4.9 V。被撤回的那句话越过了同一份文档里更新的数字，
  去取了旧的那个

**能说的是总量。接上 6 个舵机，比空载低大约 0.3 V** —— 5 V 6.2 A 的适配器空载时端子
**5.18 V**，全部接好后舵机侧空闲读到 **4.9 V**。说「大约」是因为精度只到这里：5.18 V 是万用表、
2026-09-18 量的，4.9 V 是舵机自己的 `Present_Voltage`、2026-09-14 读的，**不是同一天、也不是同一
件仪器**，而且 `Present_Voltage` 的刻度是 0.1 V。

**这 0.3 V 落在哪里，没有分开。** 可能是线路和接插件的压降，也可能是适配器自身的负载跌落。
加上负载只会把端子电压拉低、不会抬高，所以线路那一侧**至多**是这个数。

**这解释不了 7.4 V 那边的 0.3-0.4 V。** 电压轨不同、电流也不同，不能直接比。要比，就得
**在接着舵机的状态下**，同一时刻把适配器端子和舵机侧都读一遍（还没做，万用表碰一下的事）。
