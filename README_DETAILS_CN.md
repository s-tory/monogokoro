...
**舵机读到的 7.0-7.1 V，比电源输出端的 7.4 V 低 0.3-0.4 V。** 5 V 时是同一个形状：接上 6 个舵机，
比空载低**大约 0.3 V** —— 端子 5.18 V（万用表，2026-09-18）对舵机侧 4.9 V（`Present_Voltage`，
2026-09-14）。不是同一天、也不是同一件仪器，而且那个寄存器的刻度是 0.1 V，所以说「大约」。

**两边都还没定位。** 我们认为是线路和连接器上的压降，但要跟适配器自身的负载跌落分开，就得
**在接着舵机的状态下，同一时刻量电源输出端和舵机侧** —— 万用表碰一下的事，还没做。
（此处原先写着 5 V 时几乎没有压降，用的是换掉之前那只适配器的数值，2026-09-18 撤回。）
