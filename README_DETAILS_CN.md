...
**接上 6 个舵机，比空载低大约 0.3 V** —— 端子 5.18 V（万用表，2026-09-18）对舵机侧 4.9 V
（`Present_Voltage`，2026-09-14）。不是同一天、也不是同一件仪器，而且那个寄存器的刻度是 0.1 V，
所以说「大约」。**这 0.3 V 落在哪里（线路，还是适配器自身的负载跌落）没有分开。**
它也解释不了 7.4 V 那边的 0.3-0.4 V：电压轨和电流都不同。
