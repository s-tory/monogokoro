...
Then teleoperate or record with `--robot.type=so101_impedance_follower`; both fill in per-joint K/D
...
| per-joint K           | **3/8/3/1.3/1/1**                            | chosen blind by hand on the 7.4 V supply with the cerebellum on. The old route -- read the PWM held at K=1 as each joint's gravity+friction duty -- was dropped: a joint at rest cannot tell balance from static friction    |
