...
というわけでここには 3 つの層があり、それぞれが今いる場所にいる理由がある。
...
最初と最後のあいだの約 13 倍という隔たりは、おおむね生物が回している比率であり、制御則が Python に
...
ここに挙げた数値のいくつかは、ドキュメント上の値や直感的な値が間違っていると判明した後、実機で決着を
つけたものだ。この個体・この機材に固有の値であり、別の環境では測り直す価値がある。
...
- **小脳は実機で未検証。** 数学は CPU リファレンスと突き合わせてあり、シェーダは学習を通じてステップ
  単位でそのリファレンスと相互検証してあるが、実アームでの垂れ下がりの測定はまだ取れていない。示すべき
  主張は狭く反証可能である: フィードフォワードを入れると、**K を変えないまま**チェッカのテーブルの
  `err` が縮むはずだ。K を上げたときしか改善しないなら、フィードフォワードは何もしていない。
